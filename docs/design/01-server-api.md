...
1. 停止 accept；
2. 对每条连接：停止入站投递 → 尽力 flush 已入队的出站消息 → 拆除 socket →
   `OnClose(c, ErrServerClosed)`；
3. **socket 全部拆除**后返回。`ctx` 到期则放弃剩余的 flush，直接关闭。

第 3 步等的是 socket 拆除，**不是 `OnClose` 全部执行完**。带在途 `AsyncDo` 的连接
其 `OnClose` 要等各自的 `f` 结束（见下），等它们会让一个不返回的业务任务把
`Shutdown` 一路拖到 `ctx` 到期——那正是这一节要避免的。要等到最后一次 `OnClose`
用 `Wait()`。
...
并结束当前 compound 分组）；入站则是 loop 私有字段的即时切换。

> 这条决策一度被写成「调用约束从『事件循环上』**放宽**到『串行域内』——`AsyncDo`
> 的函数体也是合法调用点」。**那是错的，已撤销。** `AsyncDo` 的函数体属于
> **业务状态**意义上的串行域，却跑在另一个 goroutine 上，而入站 cipher 是 loop
> 私有状态——同一时刻 loop 可能正因超时、`Shutdown` 或写失败在拆这条连接。
> 必要条件始终是「事件循环线程上」，barrier 解决的是它**不充分**的那一半。
> 在 `AsyncDo` 里换密钥要用 `Post` 排回去（见[上面的例子](#setcipher事件循环线程上调用出站经队列-barrier-生效)）。
