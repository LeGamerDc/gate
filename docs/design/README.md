...
→ WebSocket → server 装配 → 性能护栏的顺序，每层一个 commit），随后经过**两轮**
独立评审并修完全部 P0/P1。第二轮纠出的最有价值的一类，是我在第一轮修复里
**方向修反了**的两处——`AsyncDo` 在回调失败出口应当取消而不是启动、
`Shutdown` 不该等在途任务——两者都能在规格原文里直接查到。
...
照着它们走。而且它们有一个共同的来历：**上面那张勘误表里的每一条，当初都只改
在了评审指出的那一处，没有扫全文**。于是同一个已经否掉的说法，在别的地方原封
不动地留着，还带着示例代码：

| 已勘误的结论 | 但仍留着旧说法的地方 |
| --- | --- |
| `SetCipher` / `Pause` / `AsyncDo` 只能在事件循环线程上调用 | 01 的 `SetCipher` 详解一节（含一段「从 `AsyncDo` 里直接调」的示例代码）、01 的 `Pause` 语义列表、04 的「天然在 loop 上」、`doc.go` 的落地页、`ExampleConn_AsyncDo` |
| `CloseLinger` 零值 = 1s，`Unlimited` = 立即关闭 | `config.go` 里 `CloseLinger` 字段的 **godoc**（写着「0 表示立即关闭」，正好相反） |
| `ConnsOverHighWater` 按水位穿越的边沿维护 | `stats.go` 里该字段的 **godoc**（写着「读取时为 0」） |

两处在公开 API 的 godoc 上，一处是可运行的示例——照着做会得到相反的行为或一个
